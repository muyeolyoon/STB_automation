    ...
    def save_filtered_logs(self):
        ...
        with open(self.output_path, 'w', encoding='utf-8') as outfile:
            process = subprocess.Popen(
                ["adb", "-s", AirtelSetting.device_ip, "logcat", "-v", "time"],
...
class PostAdAction:
    ...
    def _verify_channel_switch(device_ip: str, timeout: int = 15):
        proc = subprocess.Popen(
            [AirtelSetting.adb_path, "-s", device_ip, "logcat", "-v", "time"],
        ...
        try:
            for line in process.stdout:
                if "AdPlayItem" in line:
                    ...
                    LocalNotifier.send_file(recording_file, title=f"{self.actions[self.current_action_index]} 광고 녹화")
                    return True
...
def main():
    ...
    monitor = AdMonitor(AirtelSetting.device_ip, None, ad_schedule, action_sequence)
    monitor.start()
