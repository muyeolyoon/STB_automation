...
def start_log_capture():
    ...
    subprocess.run(
        [Setting.adb_path, "-s", Setting.device_ip, "logcat", "-c"],
    ...
    def capture_and_filter():
        with open(filtered_log_file, "w", encoding="utf-8") as f:
            proc = subprocess.Popen(
                [Setting.adb_path, "-s", Setting.device_ip, "logcat"],
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=True,
                encoding="utf-8",
...
def send_api_endpoint(endpoint):
    command = f"{Setting.adb_path} shell am broadcast -a tv.anypoint.agent.app.CHANGE_TEST_PROPERTY --es change.command CHANGE_API_ENDPOINT --es api.endpoint {endpoint}"
...
def reboot_device():
    print("디바이스 재부팅 중...")
    run_adb_command(f"{Setting.adb_path} reboot")
    time.sleep(10)
    wait_for_device()
...
def wait_for_device():
    print("디바이스 연결 대기 중...")
    while True:
        output = run_adb_command(f"{Setting.adb_path} get-state")
        if "device" in output:
            print("디바이스 연결 완료")
            break
        time.sleep(2)
...
def send_asset_update_command(device_id):
