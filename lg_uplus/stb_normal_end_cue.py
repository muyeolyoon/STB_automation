...
def start_recording(filename):
    ...
    return subprocess.Popen(["adb", "-s", Setting.device_ip, "shell", "screenrecord", f"/sdcard/{filename}"]), path

def stop_recording(proc, filename):
    proc.terminate()
    time.sleep(1)
    subprocess.run(["adb", "-s", Setting.device_ip, "pull", f"/sdcard/{filename}", os.path.join(RECORD_DIR, filename)])
    # subprocess.run(["adb", "-s", Setting.device_ip, "shell", "rm", f"/sdcard/{filename}"])
