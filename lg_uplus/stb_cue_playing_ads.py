...
POST_PATTERN = re.compile(r"POST http://.*/v3/device/impression-logs", re.I)
STATUS_PATTERN = re.compile(r"200 OK http://.*/v3/device/impression-logs", re.I)
...
def adb(*cmd):
    return subprocess.run(["adb", "-s", Setting.device_ip, *map(str, cmd)], capture_output=True)
...
def sheet_tab_name():
    return datetime.now().strftime("%y%m%d 모니터링")
